...
- `data-client` is a **site id**, and sites are rows in the consultant's own `clients` table (`role='site'`, `parent_id` = account); the primary site *is* the account. linart.club uses **27**, the primary site ("LinArt Support AI", own logo/accent). Site **105** is a separate child site that was created but never configured — it still returns the platform defaults. The id is not a secret (it ships in the `<script>` tag), so `deploy.sh` and `docker-compose.yml` both fall back to `27`; env still overrides.
